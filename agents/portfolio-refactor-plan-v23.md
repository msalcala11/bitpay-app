...
## 0. What changed from v22

This revision keeps the v22 architecture intact and applies the final self-consistency patches from the latest review round. The goal is to align type contracts with prose, remove unsafe helper call shapes, and pin ownership of the few remaining underspecified APIs.
...
export type WeightedGroupRateSeries = Readonly<{
  ...
  // baselineUnits, memberRateSourceKeys, and every emitted point's ts,
...
export type PopulateCheckpoint = Readonly<{
  ...
  // but the checkpoint must carry enough information to prove internal
  // consistency before resume.
  cursor?: unknown;
...
export function reconcileManifestAndQueueAgainstPopulateEligible(
  ...
  // - prune active only after the populate loop is quiesced, or leave active for
  //   the loop to observe cancellation before this helper is called again;
...
An empty scope is not a ready chart. It means the route has no visible wallets after livenet/deleted/visibility filtering. The UI should render the product’s empty/no-portfolio state and should not mount an empty chart. `hasEverPublishedValidSeries` flips true only when recompute actually publishes a non-empty valid `Series` for that scope and then persists across later incremental refreshes; `refreshing` is set while populate, rate refresh, or recompute work intersects that scope and clears when the publish lands.
...
2. If a wallet has an active invalid-history marker, do not enqueue it on unhide. If the marker has expired, remove the stale manifest invalid-history entry and enqueue the wallet so the populate loop can retry.
...
7. **Weighted full-point fingerprint test:** mutate a middle `WeightedGroupRatePoint` with unchanged endpoints; the fingerprint must change and the Exchange Rate scrub UI must re-render.
8. **Weighted zero-baseline test:** `baselineUnits === 0` or `groupIndex(t0) <= 0` yields undefined `weightedRate` / `weightedPercent` and the portfolio-entered Exchange Rate screen renders the specified empty state, not a hidden representative fallback.
...
12. **Checkpoint validity test:** each invalidity condition in the checkpoint checklist causes wallet-only staging clear and safe restart, while committed completed-wallet manifest data remains visible.
...
22. **Helper contract tests:** `resetSharedPortfolioStateForDebugClear` resets every v23 state field/tick, `startPopulate` builds/appends through the pinned API, `getSeriesIdlePoint(series)` returns the final point, and `lastAccessedAt` changes only through scoped/wallet touch semantics.
