...
## 0. What changed from v21

This revision keeps the v21 architecture intact and applies the final implementation-locking patches from the latest review round. The goal is to remove remaining ambiguity, not to add a new architecture.
...
5. **Row-shell fiat values have precise missing-rate behavior.** `currentFiatValue` is undefined if any visible nonzero member wallet lacks its live rate; zero-unit members do not block the aggregate.
...
The dedicated rate-fetch runtime is chosen deliberately. It is simpler than preserving a branch matrix and avoids runtime-global dispatch-context clobbering between populate signing context and rate fetch context. In shorthand, v22 uses three portfolio worklet runtimes: compute, populate, and rate-fetch.
...
export type WeightedGroupRatePoint = Readonly<{
  ...
  // renders for portfolio-entered collapsed groups.
  weightedRate: number | undefined;

  // No-transaction parity oracle. Derived as
  // (groupIndex(t) - groupIndex(t0)) / groupIndex(t0) * 100. Undefined when
  // baseline units or baseline index are zero.
...
export type WeightedGroupRateSeries = Readonly<{
  ...
  // collapsed groups such as USDC across chains. Weighted series are built only
  // for portfolio-entered groups with more than one distinct FiatRateAssetRef;
  // single-source groups reference the ordinary market rate series.
  memberRateSourceKeys: readonly string[];
  weighting: 'singleSource' | 'baselineUnitWeightedCollapsedGroup';
...
currentFiatValue = nonzeroMembers.every(wallet =>
...
export type AssetGroupSlice = Readonly<{
  ...
  // Multi-rate-source groups use baseline-unit weighted market-index rate sources.
...
function computeInitialScopeReadiness(args: {
  ...
  const {scopeWalletIds, manifest, previous, refreshing} = args;
  ...
  const hasEverPublishedValidSeries =
    previous?.hasEverPublishedValidSeries === true || initialScopeReady;
...
If `baselineUnits === 0` or `groupIndex(t0) <= 0`, `weightedRate` and `weightedPercent` are undefined for that interval. The portfolio-entered weighted Exchange Rate screen should render an empty state such as “Not available for this interval; no holdings at the start of the window.” It may offer a separate “View market rate” action to a normal single-source market route, but it must not silently substitute a representative constituent series.

Build the weighted series only for collapsed groups with more than one distinct `FiatRateAssetRef`. Single-source groups reuse the ordinary market rate series and ordinary Exchange Rate route. Weighted group series are derived render state; do not persist them in MMKV.
...
A portfolio row tap for a collapsed multi-rate-source asset group uses `kind: 'portfolioWeightedAssetGroup'`. A standalone Exchange Rates screen uses `kind: 'marketAsset'`. This is load-bearing: opening one arbitrary token deployment from a collapsed `usdc` portfolio row would silently break the row/detail/exchange-rate parity mental model. UI labels should make this clear, for example “Portfolio-weighted USDC rate” when multiple rate sources are present.
...
Pin the exact expected percent in tests. This catches single-token fallback, wrong weighting, missing constituent rates, and incorrect quote-bridge handling.
...
v22 fixes the canonical quote to `USD`:
...
3. Run `ensureFresh` for visible asset groups plus canonical `BTC/USD`.
...
export async function onWalletsDeleted(args: {walletIds: readonly string[]}): Promise<void> {
  ...
  const survivingPending = loadQueue()?.pending.map(i => i.walletId) ?? [];
  if (survivingPending.length) {
    populateWallets(survivingPending, {reason: 'manual'});
  }
}
```
...
| `onWalletsDeleted` | guard → cancel/wait populate → guard → reconcile → clear snapshots → guard → full recompute with `evictScopedWalletIds` → re-kick survivors | Prevents populate/write races and stale scoped entries. |
...
- Confirm whether `rate:v1:*` is shared with Exchange Rate screens. v22 default is not to wipe it.
...
- No-transaction PnL percent equals the ordinary Exchange Rate percent for single-rate-source assets and equals the baseline-unit weighted group exchange-rate percent for collapsed multi-rate-source asset groups, including the pinned `180.392 → 180.592 → 0.1108696616%` fixture.
...
54. Row shell `currentFiatValue` is computed by summing each visible nonzero member wallet through its own `FiatRateAssetRef` / live-rate key, not one coarse group rate unless all nonzero members share the same source; missing rate for a nonzero member yields `undefined`, while missing rate for a zero-unit member does not block the aggregate.
...
### V22 lock-blocker tests and anti-regression variants
...
7. **Weighted zero-baseline test:** `baselineUnits === 0` or `groupIndex(t0) <= 0` yields undefined `weightedRate` / `weightedPercent` and the portfolio-entered Exchange Rate screen renders the specified empty state, not a hidden representative fallback.
8. **Weighted numeric fixture:** the `180.392 → 180.592 → 0.1108696616%` fixture is pinned within `1e-10` tolerance.
9. **Urgent same-wallet supersession test:** incoming urgent send/pull item drops older unstarted same-wallet normal/background pending items, preserves existing urgent items, and does not preempt an active same-wallet item.
10. **Stable priority insertion test:** insertion preserves FIFO within each priority class: existing urgent, incoming urgent, existing normal, incoming normal, existing background, incoming background.
11. **Checkpoint validity test:** each invalidity condition in the checkpoint checklist causes wallet-only staging clear and safe restart, while committed completed-wallet manifest data remains visible.
12. **Wallet/delete triple-guard race tests:** guard #2 flips during populate wait; guard #3 flips during snapshot clear; both return before later side effects.
13. **MMKV registry test:** seed one registered key and one unregistered real MMKV key; wipe deletes both through registry-aware delete and leaves `kvStore.listKeys()` clean.
14. **Manifest schema validation test:** invalid/missing schema or malformed JSON returns `null` and logs once; no business-logic silent migration.
15. **Logger contract test:** `logPortfolioRuntimeError` never throws, never returns a Promise, includes `subsystem: 'portfolio-v2'`, and preserves `extra.tag`.
16. **Hide Crypto Balances orthogonality test:** dispatch `toggleHideAllBalances()` twenty times and assert zero runtime calls, zero MMKV writes, zero trigger invocations, zero `sharedPortfolioState` writes, and only UI re-renders.
17. **Per-trigger ordering test:** table order is enforced; post-auth is the only warm-publish-first trigger, and pull/live/send/quote/delete/show-toggle follow their explicit orders.
