...
100. **Populate Nitro boundary test:** instrument JS txhistory/signing/request helpers and run populate. Assert txhistory signing, request fetching, pagination, and response processing occur through the worklet/Nitro hybrid-object path, with zero JS-thread txhistory request/signing calls. Anti-regression variant: stub populate to call a JS txhistory/signing helper and assert the test fails.
