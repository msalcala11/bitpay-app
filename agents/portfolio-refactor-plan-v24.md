# Portfolio runtime refactor plan — v24

**Status:** implementation-ready contract-polish revision of v23. This version keeps the v23 architecture locked and only tightens edge-case contracts: row-shell all-zero fiat behavior, weighted-rate fingerprint identity, per-interval zero-baseline weighted routes, JSON-validated checkpoint cursors, non-optional checkpoint failure booleans, and two additional blocker tests.
...
This revision keeps the v23 architecture intact and applies a small contract-polish patch from the latest review round. The goal is not to redesign the system; it is to remove the last edge-case ambiguities that could cause implementation drift.
...
5. **Row-shell fiat values have precise all-zero and missing-rate behavior.** No visible members means no shell. Visible members with all zero current units publish `currentFiatValue: 0`. Any visible nonzero member lacking a live rate makes `currentFiatValue` undefined; runtime never publishes a partial fiat sum.
...
14. **Checkpoint type and validity checklist match.** `PopulateCheckpoint` now includes schema/run/item identity, staging keys, snapshot revision, JSON-serializable/schema-validated cursor data, ingest/page compatibility, and non-optional failure state fields.
...
20. **Final blocker tests cover all-zero shell fiat and per-interval zero-baseline weighted routes.** These are small tests, but they prevent common implementation shortcuts.
...
The dedicated rate-fetch runtime is chosen deliberately. It is simpler than preserving a branch matrix and avoids runtime-global dispatch-context clobbering between populate signing context and rate fetch context. In shorthand, v24 uses three portfolio worklet runtimes: compute, populate, and rate-fetch.
...
export type WeightedGroupRateSeries = Readonly<{
  ...
  // the complete baseline weight vector (baselineUnitsByRateSourceKey),
  // memberRateSourceKeys, and every emitted point's ts, weightedRate, and
  // weightedPercent. Endpoint-only fingerprints are forbidden because the
  // Exchange Rate scrub UI can stale on middle-point mutations. The weight
  // vector is part of the identity of the weighted market index even if two
  // emitted point arrays happen to match numerically.
  fingerprint: string;
  interval: Interval;
  windowStartTs: number;
  windowEndTs: number;
  ...
  // Interval-local unavailable state. The route can remain valid overall even
  // if one interval has no meaningful weighted rate because holdings at that
  // interval baseline were zero. Timeframe switching may reveal another interval
...
Weighted group rate fingerprints use the same full-point rule and also hash the weight vector:
...
weightedGroupRateFingerprint = stableHash([
  ...
  ...points.flatMap(point => [
    point.ts,
    stableNumberOrNull(point.weightedRate),
    stableNumberOrNull(point.weightedPercent),
  ]),
]);
```
...
v24 keeps the canonical quote fixed to `USD`:
...
if (baselineUnits <= 0 || groupIndex(t0, targetQuote) <= 0) {
  weightedRate(t, targetQuote) = undefined;
  weightedPercent(t, targetQuote) = undefined;
} else {
...
The zero-baseline guard is interval-local. A `portfolioWeightedAssetGroup` route remains valid overall even when a specific interval publishes `unavailableReason: 'zeroBaseline'`; the UI renders that interval's weighted-rate chart as unavailable while allowing timeframe switches to intervals with a valid baseline. It must not auto-navigate to a representative `marketAsset` fallback.
...
2. If a wallet has an active invalid-history marker, do not enqueue it on unhide.
3. If the marker is expired or absent, enqueue the wallet. The trigger does not directly mutate invalid-history manifest entries; at item start, the populate loop re-checks the persisted marker, clears expired invalid-history manifest state through the normal populate path, and retries if eligible.
4. Schedule a full recompute using visibility-respecting eligible wallets.
...
- Confirm whether `rate:v1:*` is shared with Exchange Rate screens. v24 default is not to wipe it.
...
### V24 lock-blocker tests and anti-regression variants
...
7. **Weighted full-point and weight-vector fingerprint test:** mutate a middle `WeightedGroupRatePoint` with unchanged endpoints; the fingerprint must change and the Exchange Rate scrub UI must re-render. Then change `baselineUnitsByRateSourceKey` while keeping emitted endpoints unchanged; the fingerprint must still change because the weight vector is part of the index identity.
8. **Weighted zero-baseline interval test:** for a valid `portfolioWeightedAssetGroup` route, an interval with `baselineUnits === 0` or `groupIndex(t0) <= 0` publishes interval-local `unavailableReason: 'zeroBaseline'` and renders the specified empty state; switching to an interval with nonzero baseline renders the weighted series. No fallback navigation to `marketAsset` occurs.
...
22. **Helper contract tests:** `resetSharedPortfolioStateForDebugClear` resets every v24 state field/tick, `startPopulate` builds/appends through the pinned API, `getSeriesIdlePoint(series)` returns the final point, and `lastAccessedAt` changes only through scoped/wallet touch semantics.
23. **All-zero shell fiat test:** visible member wallets exist, every current unit amount is zero, and one or more zero-unit rates may be missing; runtime still publishes a row shell with `currentFiatValue: 0`. A no-visible-members fixture publishes no shell.
24. **Weighted route per-interval zero-baseline test:** a collapsed route remains `kind: 'portfolioWeightedAssetGroup'` when one interval is unavailable due to zero baseline, that interval renders the weighted-rate empty state without crashing, and switching to another valid interval renders normally without opening a representative market route.
