...
3. Rate ↔ PnL consistency, including the "no transactions in window => PnL % equals Exchange Rate %" case.
   ...
   Include a named **same-user transfer fixture** with concrete pinned values so transfer-netting bugs fail on a specific number rather than a vague "not net zero" assertion:
   - **Setup:** wallet A and wallet B are both user-owned. A starts the window holding `100 USDC` with `$100.00` accumulated cost basis (i.e., A's average basis rate is `$1.00`). B starts the window with `0 USDC`, `$0.00` basis. The window is `1D`. The collapsed `assetGroupId = 'usdc'` covers both wallets.
   - **Event:** at the transfer timestamp τ inside the window, the USDC spot rate is `$1.005`. A sends `40 USDC` to B. A's balance delta is `-40 USDC`, B's is `+40 USDC`.
   - **Per-wallet expected end state:**
     - A: `60 USDC`, remaining cost basis = `$100.00 * (60 / 100) = $60.00` (pro-rata disposal at A's average rate).
     - B: `40 USDC`, remaining cost basis = `40 * $1.005 = $40.20` (add basis at the receiving event's spot rate).
   ...
   - **Cross-chain variant:** rerun the same fixture with A on Ethereum and B on Polygon, both `assetGroupId = 'usdc'`, with per-chain rates `$1.005` (ETH-side at τ) vs `$1.0048` (Polygon-side at τ). A's disposal uses A's average basis rate; B's add-basis uses Polygon's `$1.0048`. Aggregate remaining cost basis = `$60.00 + 40 * $1.0048 = $60.00 + $40.192 = $100.192`. This pins the per-chain spot-rate sourcing rule from the Phase 3 transfer/accounting model note — collapsing the two wallets under one `assetGroupId` does not erase the per-chain rate distinction at event time.
