...
> - **`AssetRow` / `AssetRowV2`:** when masked, crypto-amount, fiat-amount, and delta-fiat all render via `maskIfHidden(true, ...)`. Skeleton placeholders are bypassed: if a row would normally show a skeleton during populate, show `'****'` instead. PnL percent follows the same rule. Readiness-skeleton logic only applies when `hideAllBalances` is false.
...
> - **`AssetBalanceHistoryScreen`:** mask the header + PnL subtext; hide the chart. Exchange Rate detail screen is NOT a portfolio surface and does not consume this flag — it continues showing rates regardless.
...
> - Apply the mask to the Home **Exchange Rates** section or the **Exchange Rate** detail screen. Those surfaces show market prices, not user balances — they are unaffected by `hideAllBalances` in v1 and must remain so in v2. The `maskIfHidden(...)` call only wraps balance / fiat-amount / PnL-delta / PnL-percent strings, not rate values.
