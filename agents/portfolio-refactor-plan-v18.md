...
> - **Point cap.** Every emitted balance chart series has `series.points.length <= MAX_CHART_POINTS` (`89`). If the resolved window has 89 or fewer sample timestamps, emit them all. If it has more, downsample on the compute runtime to at most 89 before publishing, while preserving the first and final resolved window endpoints. This applies uniformly to Home total, wallet, account, key-scoped, asset-group detail, and every `scopedByWalletSet` series. Do not push dense balance-chart arrays to JS and trim them in React — the published render payload itself is capped.
>
