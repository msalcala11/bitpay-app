...
> - For valid token wallets, `chain` and `tokenAddress` are both populated. Native coins leave both fields `undefined` so they route through path (1) above. The helper tolerates a misbehaving caller that passes `tokenAddress` without `chain` (the normalization may strip `tokenAddress` in that case), but v2 callers should always supply both fields for tokens.
>
...
>    - (a) The classifier collapses `wbtc → btc` and legacy ETH-side MATIC → native `pol`, so after `normalizeUniqueFiatRateAssets` the native-coin set (compared as a `Set<string>`, order-insensitive) equals `{'btc', 'eth', 'sol', 'pol'}`.
