...
>
> **Cost-basis method (display PnL, not tax accounting):** v18 uses a window-local average-cost-style remaining basis. At the window baseline, `remainingCostBasisFiat = unitsAtWindowStart * baselineRate`. Inflows add basis at the event spot rate. Outflows dispose basis pro rata via `remainingCostBasisFiat *= afterUnits / beforeUnits`, equivalent to removing units at the wallet's current window-local average basis. This is not tax-lot accounting, FIFO, LIFO, realized PnL, or historical acquisition-cost tracking.
>
