...
>   if (!canRunPortfolioV2Work()) return;
>   // Same shape as populateWallet, but appending a batch. Each walletId flows
>   // through idempotent `appendToQueue` (see contract below).
> }
>
...
> - `appendToQueue(walletId)` is idempotent with respect to walletIds already present in `queue.remainingWalletIds`. It does NOT append duplicates and does NOT bump `orderRevision` when the resulting set of ids is unchanged. This matches the `buildQueue` monotonicity invariant (guardrail #14): `orderRevision` advances only when the ordered walletId set actually changes.
> - `populateWallets(walletIds)` is therefore the correct "kick the existing reconciled queue" helper when callers pass walletIds already in `queue.remainingWalletIds` (e.g., `onWalletsDeleted`'s post-reconcile survivors). It is NOT a "rebuild the queue from these ids" helper — pass walletIds that belong in the current queue, not a fresh replacement set.
> - `appendToQueue` only bumps `orderRevision` for genuinely new walletIds that introduce new ordering. Passing exclusively already-queued ids is strictly a "clear cancel flag + runPopulate kick," which is the semantic `onWalletsDeleted`, `onSendCompleted`, and `onPullToRefresh` all rely on.
> - Callers that want a fresh queue (e.g., `onShowPortfolioVisibilityChanged` on re-enable) go through `startPopulate({isFirstPopulate: true, ...})`, which calls `buildAndSaveQueue` — not `populateWallets`.
>
...
> - `populateWallets` / `appendToQueue` idempotency regression (Phase 5 API contract): seed the queue with `remainingWalletIds = [A, B, C]` and a specific `orderRevision = 5`. Call `populateWallets(['A', 'B'])`. Assert: (a) `remainingWalletIds` is still `[A, B, C]` with no duplicates of A or B; (b) `orderRevision` is still 5 — no bump, because the queue's ordered id set is unchanged; (c) the populate loop is kicked (cancel flag cleared, `runPopulate` dispatched); (d) `populateWallets([])` (empty) does not duplicate queue state and does not fire `runPopulate`. Regresses against a naive `appendToQueue` that pushes duplicates or bumps `orderRevision` on idempotent calls. Paired test: `populateWallets(['D'])` with `D` not in the current queue DOES append and bumps `orderRevision` to 6 — proving the idempotency is scoped to already-queued ids, not a blanket no-op.
