...
>
> **Transfer/accounting model (load-bearing v1 parity):** for display PnL, every positive wallet balance delta is treated as an inflow / buy and every negative wallet balance delta is treated as an outflow / sell. V18 does **not** identify, net, or exclude transfers between wallets owned by the same user. The formula is intentionally wallet-local: a same-asset movement from user-owned wallet A to user-owned wallet B applies pro-rata disposal in A and adds cost basis in B at the receiving event's rate. Do not add owned-wallet transfer matching unless product explicitly changes the PnL model.
>
