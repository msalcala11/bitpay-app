...
5. **Refresh queue insertion is stable and supersedes stale work.** Urgent send/pull work runs after the active wallet, preserves FIFO within each priority class, and drops older unstarted same-wallet normal/background items so stale pending work does not run after fresher urgent work.
...
22. **Helper contract tests:** `resetSharedPortfolioStateForDebugClear` resets every current state field/tick, `startPopulate` builds/appends through the pinned API, `getSeriesIdlePoint(series)` returns the final point, and `lastAccessedAt` changes only through scoped/wallet touch semantics.
