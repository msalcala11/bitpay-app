...
## 0. What changed from v20

This revision applies the final implementation-locking feedback on v20. The v20 architecture remains intact; v21 tightens the edge contracts that could otherwise be interpreted in mathematically or operationally inconsistent ways.

1. **Collapsed multi-chain no-transaction parity is clarified.** Single-rate-source assets still compare directly to their normal Exchange Rate series. Collapsed multi-rate-source asset groups compare to a runtime-published weighted group exchange-rate series derived from the same constituent rate sources and wallet weights.
2. **Refresh queue priority is explicit.** Send refreshes and pull-to-refresh changed-wallet refreshes are `urgentUserVisible` and are inserted at the front of `pending`, meaning they run immediately after the active wallet completes rather than waiting behind the remaining initial/background queue.
3. **Row-shell fiat values are per-member rated.** `currentFiatValue` sums each visible member wallet using that wallet’s own `FiatRateAssetRef` / live-rate key. It does not use one coarse ticker-group rate unless every member wallet shares the exact same rate source.
4. **The pinned same-user transfer fixture is restored.** The `$100.20` same-chain and `$100.192` cross-chain USDC transfer fixtures are product-oracle tests so transfer-netting or wrong per-chain rate sourcing fails on exact numbers.
5. **Scoped cache bounds are concrete.** `MAX_SCOPED_CACHE_ENTRIES = 8`, with LRU/touch semantics, current-scope protection, delete-first pruning, refresh-before-eviction ordering, and tests for cap behavior.
6. **`usePortfolioSlice` comparators are worklet-safe.** Arbitrary JS closures are not allowed as equality functions. Typed hook façades use named worklet comparators such as `seriesFingerprint`, `rowFingerprint`, and `shallowScalar`.
7. **Show Portfolio rapid-toggle behavior is latched.** An OFF-created wipe obligation must complete before any later ON can start populate, even if last-toggle-wins means the final visible setting is ON.
...
The dedicated rate-fetch runtime is chosen deliberately. It is simpler than preserving a branch matrix and avoids runtime-global dispatch-context clobbering between populate signing context and rate fetch context. In shorthand, v21 uses three portfolio worklet runtimes: compute, populate, and rate-fetch.
...
export type RatePoint = Readonly<{
  ts: number;
  rate: number;
...
export type WeightedGroupRateSeries = Readonly<{
  ...
  windowEndTs: number;
  sampledFromStoredInterval: StoredRateInterval;
  // One source for single-rate-source groups; multiple FiatRateAssetRef keys for
  // collapsed groups such as USDC across chains.
  memberRateSourceKeys: readonly string[];
  weighting: 'singleSource' | 'valueWeightedCollapsedGroup';
...
```ts
currentFiatValue = sum(
  visibleMemberWallets.map(wallet =>
    currentUnits(wallet) * liveRateFor(wallet.fiatRateAssetRef)
  )
);
```

The runtime performs this aggregation. The UI may format, mask, navigate, or render skeletons, but it must not aggregate through `memberWalletIds` or walk wallet balances/points to derive row amounts. This is especially important for collapsed ticker groups and native/token distinctions.
...
export type AssetGroupSlice = Readonly<{
  ...
  // Exchange-rate comparison series for this grouped asset scope.
  // Single-rate-source groups reduce to the underlying Exchange Rate series.
  // Multi-rate-source groups are value/basis-weighted from member rate sources.
...
A new run may enqueue the same wallet even if an earlier run already completed that wallet. If the same wallet is already pending under an older run, v21 still allows the new run-scoped item. A future optimization may safely coalesce an older unstarted item into a newer item for the same wallet, but that optimization is not required and must preserve send/pull correctness.

### Queue insertion priority

Do not interrupt a wallet that is already active. The active wallet finishes or yields through the normal cancellation path. Priority controls where new items go in `pending`, so urgent user-visible work runs next after the active wallet completes.
...
```txt
urgentUserVisible   inserted before existing pending items
normalUserVisible   inserted after existing urgent items but before background tail if simple to do; appending after all pending is acceptable only for non-refresh imports/manuals
background          appended to the tail
```
...
function insertPendingItemsByPriority(
  ...
  const urgent = incoming.filter(i => i.priority === 'urgentUserVisible');
  const normal = incoming.filter(i => i.priority === 'normalUserVisible');
  const background = incoming.filter(i => i.priority === 'background');
  if (!urgent.length && !normal.length) return [...existing, ...background];

  const existingUrgent = existing.filter(i => i.priority === 'urgentUserVisible');
  const existingRest = existing.filter(i => i.priority !== 'urgentUserVisible');
  return [...existingUrgent, ...urgent, ...normal, ...existingRest, ...background];
}
```

### Refresh priority contract

Do not let user-visible refresh work sit behind a long first-populate tail. `send` and pull-to-refresh changed-wallet items are `urgentUserVisible`. They are inserted at the front of `pending`, which means immediately after the current active wallet. The active wallet is not preempted mid-page unless a future design explicitly introduces cancellation/preemption. Background initial/app-launch work remains behind urgent items.
...
For a **collapsed multi-chain / multi-rate-source asset group** such as `usdc` across Ethereum, Polygon, and Solana, the aggregate no-transaction PnL percent is a value/basis-weighted blend of each constituent rate source, not necessarily equal to one arbitrary USDC exchange-rate series.
...
collapsed multi-rate-source asset group:
  portfolio no-tx pnlPercent === runtime-published weighted group exchange-rate percent
```

The weighted group exchange-rate series is built by the compute runtime from the same visible member wallets and `FiatRateAssetRef` sources used for the collapsed asset-group portfolio series. At each boundary/sample timestamp, it weights each constituent rate source by that member wallet's baseline value or remaining basis contribution for the selected window, matching the row/detail PnL denominator. If a portfolio-entered Exchange Rate detail screen represents a collapsed multi-rate-source group, it must use this weighted group series. If the user opens a standalone market Exchange Rate screen for a single token deployment, parity applies only to the matching single rate source.
...
v21 fixes the canonical quote to `USD`:
...
- builds weighted group exchange-rate series for every visible asset group;
...
- `currentFiatValue` is computed by summing each visible member wallet at that wallet's own `FiatRateAssetRef` / live-rate key; a collapsed ticker group uses a single group rate only when all members share the same rate source.
...
- Confirm whether `rate:v1:*` is shared with Exchange Rate screens. v21 default is not to wipe it.
...
- No-transaction PnL percent equals the ordinary Exchange Rate percent for single-rate-source assets and equals the weighted group exchange-rate percent for collapsed multi-rate-source asset groups.
...
33. Collapsed multi-rate-source asset groups expose a weighted group exchange-rate series, and no-transaction PnL percent equals that weighted group percent.
...
54. Row shell `currentFiatValue` is computed by summing each member wallet through its own `FiatRateAssetRef` / live-rate key, not one coarse group rate unless all members share the same source.
...
71. Expired invalid-history marker can retry and successful populate clears marker.
