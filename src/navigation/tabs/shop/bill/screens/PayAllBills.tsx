...
const PayAllBills = ({
  ...
  // Only include accounts that are actually payable to avoid index mismatches
  const payableAccounts = useMemo(
    () => accounts.filter(acct => acct.isPayable),
    [accounts],
  );
  ...
  const [amountModalAccountAndIndex, setAmountModalAccountAndIndex] = useState({
    account: payableAccounts[0],
    accountIndex: 0,
  });

  const [accountsState, setAccountsState] = useState(
    payableAccounts.map((acct, index) => ({
      expanded: index === 0,
      selectedAmount:
  ...
  const goToConfirmScreen = async () => {
    const billPayments = payableAccounts
    ...
    <>
      <ScrollView
        ...
        <SectionContainer
          ...
          {payableAccounts.map((account, accountIndex) => (
