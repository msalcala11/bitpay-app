...
describe('PortfolioEngine compute sessions', () => {
  ...
  it('computes compact asset rows directly from worker-owned storage', async () => {
    const kv = new MemoryKv();
    const engine = new PortfolioEngine(kv);
    const wallet = mkWallet({balanceAtomic: '200000000', balanceFormatted: '2'});
    const credentials = mkCreds();
    ...
    const result = await engine.computeAssetRows({
      cfg: {baseUrl: 'https://bws.invalid'},
      wallets: [
        {
          walletId: wallet.walletId,
      ...
      currentRatesByAssetId: {'btc:btc': 11000},
    });
    ...
    expect(result.rows[0]?.pnlPercentRatio).toBeCloseTo(1000 / 21000);
    expect(result.rows[0]?.displayPercentRatio).toBeCloseTo(1000 / 21000);
  });

  it('returns compact placeholder asset rows without warming rates when snapshots are unavailable', async () => {
    const kv = new MemoryKv();
    const engine = new PortfolioEngine(kv);
    const wallet = mkWallet({balanceAtomic: '100000000', balanceFormatted: '1'});
    const credentials = mkCreds();
