...
function scoreOffset(arr: RatePoint[], ref: RatePoint[], offset: number): number {
  const diffs: number[] = [];
...
function keepIndicesLTTB(series: AlignedRatePoint[], targetLen: number): number[] {
  ...
  for (let k = 1; k < out.length; k++) {
    if (out[k] <= out[k - 1]) {
      throw new Error('downsampleTimestamps: failed to compute strictly increasing indices');
    }
  }
...
export function downsampleTimestamps(
  ...
  for (const coin of coins) {
    if ((input[coin]?.length ?? 0) !== len) {
      throw new Error('downsampleTimestamps: all aligned arrays must have the same length');
    }
  }
...
export function trimTimestamps(input: AlignedRatesByCoin): AlignedRatesByCoin {
  ...
  for (const coin of coins) {
    if ((input[coin]?.length ?? 0) !== len) {
      throw new Error('trimTimestamps: all aligned arrays must have the same length');
    }
  }
  ...
  if (trimStart > 0) {
    let maxMissing = 0;
    for (const coin of coins) {
      maxMissing = Math.max(maxMissing, countMissingOnStart(input[coin], trimStart));
    }
  ...
  if (trimEnd > 0) {
    let maxMissing = 0;
    for (const coin of coins) {
      maxMissing = Math.max(maxMissing, countMissingOnEnd(input[coin], trimEnd));
    }
...
/**
 * Strict version of trimTimestamps.
 *
 * The BitPay util (trimTimestamps) intentionally keeps large amounts of null-padding
 * when the coverage mismatch is significant ("maxMissing > 2"). That behavior is
 * useful for some UI flows, but for multi-coin aggregation we often need the true
 * overlapping window.
 *
 * This function trims to the maximal contiguous overlap window (no null padding)
 * without any "keep padding" heuristics.
 */
export function trimTimestampsStrict(input: AlignedRatesByCoin): AlignedRatesByCoin {
  const coins = Object.keys(input);
  if (coins.length === 0) {
    return {};
  }
  ...
  for (const coin of coins) {
    if ((input[coin]?.length ?? 0) !== len) {
      throw new Error('trimTimestampsStrict: all aligned arrays must have the same length');
    }
  }
  ...
  let start = 0;
  while (start < len && !allHaveRateAt(start)) {
    start++;
  }

  let end = len - 1;
  while (end >= 0 && !allHaveRateAt(end)) {
    end--;
  }
  ...
  const out: AlignedRatesByCoin = {};
  for (const coin of coins) {
    out[coin] = input[coin].slice(start, end + 1);
  }
